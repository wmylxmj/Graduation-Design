...
def TCPNet(step=5, features=51, ndim=7, units=256):
    X = Input(shape=(step, features))
    lstm_layer = LSTM(units, return_state=True, kernel_regularizer=keras.regularizers.l2(0.01)) 
    dense_layer = Dense(ndim, activation='softmax')
    a0 = Input(shape=(units, ), name='a0')
    c0 = Input(shape=(units, ), name='c0')
    a = a0
    c = c0
    for t in range(step):
        x = Lambda(lambda x: X[:, t, :])(X)
        x = Reshape((1, features))(x)
        a, _, c = lstm_layer(x, initial_state=[a, c])
        pass
    Y = dense_layer(a)
    model = Model([X, a0, c0], Y)
    return model
