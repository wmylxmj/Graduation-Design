...
def CuttingToOringalShape(x, h, w):
    x = Lambda(lambda x: x[:, 0:h, 0:w, :])(x)
    return x

def UpSamplingBlock(x, filters, scale, shape=None):
    x_y = ConvBlock(x, filters=filters*scale**2, expansion=6, kernel=3)
    x_y = SubpixelConv2D(scale)(x_y)
    if shape == None:
        return x_y
    x_y = CuttingToOringalShape(x_y, shape[1], shape[2])
    return x_y

def DownSamplingBlock(x, filters, scale):
    x_y = PaddingToIntegerMultiples(x, integer=scale)
    x_y = DesubpixelConv2D(scale=scale)(x_y)
    x_y = ConvBlock(x_y, filters=filters, expansion=6, kernel=3)
    return x_y

def SAENet(layers=8, joints=17):
    ...
    for i in range(layers):
        ...
        x4_x4 = Lambda(tf.multiply, arguments={'y':g})(x4_x4)
        # upsampling x8
        x8_x4 = UpSamplingBlock(x8, filters=32, scale=2, shape=K.shape(x4))
        x8_x4 = Lambda(tf.multiply, arguments={'y':g})(x8_x4)
        # upsampling x16
        x16_x4 = UpSamplingBlock(x16, filters=32, scale=4, shape=K.shape(x4))
        x16_x4 = Lambda(tf.multiply, arguments={'y':g})(x16_x4)
        # x8
        x8_x8 = ConvBlock(x8, filters=64, expansion=6, kernel=3)
        x8_x8 = Lambda(tf.multiply, arguments={'y':g})(x8_x8)
        # upsampling x16
        x16_x8 = UpSamplingBlock(x16, filters=64, scale=2, shape=K.shape(x8))
        x16_x8 = Lambda(tf.multiply, arguments={'y':g})(x16_x8)
        # downsampling x4
        x4_x8 = DownSamplingBlock(x4, filters=64, scale=2)
        x4_x8 = Lambda(tf.multiply, arguments={'y':g})(x4_x8)
        # x16
        x16_x16 = ConvBlock(x16, filters=128, expansion=6, kernel=3)
        x16_x16 = Lambda(tf.multiply, arguments={'y':g})(x16_x16)
        # downsampling x4
        x4_x16 = DownSamplingBlock(x4, filters=128, scale=4)
        x4_x16 = Lambda(tf.multiply, arguments={'y':g})(x4_x16)
        # downsampling x8
        x8_x16 = DownSamplingBlock(x8, filters=128, scale=2)
        x8_x16 = Lambda(tf.multiply, arguments={'y':g})(x8_x16)
        # add
