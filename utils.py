...
class DataLoader(object):
    ...
    def __init__(self, images="D:/train2017", annotations="D:/annotations/person_keypoints_train2017.json", image_size=(640, 480)):
    ...
    def pair(self, image_id):
        ...
        for people in range(num_people):
            ...
            for joint in range(num_joints):
                keypoint = keypoints[people][3*joint:3*joint+3]
                keypoints_resized[people].append(int(keypoint[0]*self.image_size[0]/w))
                keypoints_resized[people].append(int(keypoint[1]*self.image_size[1]/h))
                keypoints_resized[people].append(int(keypoint[2]))
                pass
            pass
        # resize heatmaps
        heatmaps_resized = np.zeros((self.image_size[1], self.image_size[0], heatmaps.shape[2]), dtype=np.float32)
        for i in range(num_joints):
            heatmap = Image.fromarray(heatmaps[:, :, i])
            heatmap = heatmap.resize(self.image_size)
            heatmaps_resized[:, :, i] = np.array(heatmap)
            pass     
        return image_resized, keypoints_resized, heatmaps_resized
