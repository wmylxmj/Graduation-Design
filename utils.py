...
class AnnotationsLoader(object):
    ...
    def heatmaps(self, h, w, keypoints, sigma=10):
    ...
    def tapmaps(self, h, w, keypoints):
        # ms coco have 17 joints
        num_joints = 17
        num_people = len(keypoints)
        tapmaps = np.zeros((h, w, num_joints), dtype=np.uint8)
        for joint in range(num_joints):
            for people in range(num_people):
                keypoint = keypoints[people][3*joint:3*joint+3]
                # 0 means unlabeled
                if keypoint[2] == 0:
                    continue
                tapmaps[keypoint[1]][keypoint[0]][joint] = people + 1
                pass
            pass
        return tapmaps
...
class DataLoader(object):
    ...
    def pair(self, image_id):
        ...
        # tagmaps
        tapmaps = self.annotationsLoader.tapmaps(output_size[1], output_size[0], keypoints_resized)
        maps = np.concatenate([heatmaps, tapmaps], axis=-1)
        return image_resized, maps
