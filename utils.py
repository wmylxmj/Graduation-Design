...
class TCPDataLoader(object):
    ...
    def pair(self, json_path, n_classes=6):
        with open(json_path, "r") as f: 
            data = json.load(f)
            pass
        x = data['x']
        ...
        w_offset = random.sample(range(-1*int(w_min), int(153-w_max)), 1)
        h_offset = random.sample(range(-1*int(h_min), int(100-h_max)), 1)
        x = np.float32(x)
        # w
        x[:, 0::3] += w_offset
        x[:, 0::3] /= 100.0
    ...
    def batches(self, batch_size=4, complete_batch_only=False, folder="infos/train"):
