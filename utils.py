...
class TCPDataLoader(object):
    ...
    def pair(self, json_path, n_classes=6, test_mode=False, visualize=False):
        with open(json_path, "r") as f: 
            data = json.load(f)
            pass
        x = data['x']
        ...
        if test_mode==False:
            rotate_center = [76, 50]
            rotate_center[0] = rotate_center[0] + random.sample(range(-76, 76), 1)[0]
            rotate_center[1] = rotate_center[1] + random.sample(range(-50, 50), 1)[0]
            angle = random.sample(range(-25, 25), 1)[0] / 180.0 * np.pi
            ...
            angle += atan
            # flip
            x[:, 0::3] = distance * random.sample([-1, 1], 1)[0] * np.cos(angle)
