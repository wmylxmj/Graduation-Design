...
class TCPDataLoader(object):
    ...
    def prepare(self, model, dataset="G:/dataset/", cover=False):
        classes = os.listdir(dataset)
        for c in classes:
            folders = os.listdir(os.path.join(dataset, c))
            for folder in folders:
                if "{}_{}.json".format(c, folder) in os.listdir("jsons") and cover==False:
                    continue
                ...
                data["x"] = sequence
                data["y"] = int(c)
                with open("jsons/{}_{}.json".format(c, folder), "w") as f:
                    json.dump(data, f)
                    print("jsons/{}_{}.json writed.".format(c, folder))
                    pass
                pass
            pass
        pass
    
    def split_train_val(self, train_percent=0.9):
        jsons = list(filter(lambda x:x.endswith(".json"), os.listdir("jsons")))
        n_train = int(train_percent * len(jsons))
        np.random.shuffle(jsons)
        train = jsons[0:n_train]
        val = jsons[n_train:]
        if not os.path.exists("infos/train"):
            os.mkdir("infos/train")
            pass
        else:
            shutil.rmtree("infos/train")
            os.mkdir("infos/train")
            pass
        if not os.path.exists("infos/val"):
            os.mkdir("infos/val")
            pass
        else:
            shutil.rmtree("infos/val")
            os.mkdir("infos/val")
            pass
        for file in train:
            sp = os.path.join("jsons", file)
            dp = os.path.join("infos/train", file)
            copyfile(sp, dp)
            pass
        for file in val:
            sp = os.path.join("jsons", file)
            dp = os.path.join("infos/val", file)
            copyfile(sp, dp)
            pass
        pass
    ...
    def pair(self, json_path, n_classes=7, test_mode=False, visualize=False):
        ...
        if len(x) < self.n_frames:
            ...
            pass
        # cutting frames
        if len(x) >= (self.n_frames * 2) and test_mode==False:
