...
class AnnotationsLoader(object):
    ...
    def tagmaps(self, h, w, keypoints, num_joints=17):
        num_people = len(keypoints)
        tagmaps = np.zeros((h, w, num_joints), dtype=np.uint8)
        for joint in range(num_joints):
            for people in range(num_people):
                keypoint = keypoints[people][3*joint:3*joint+3]
                # 0 means unlabeled
                if keypoint[2] == 0:
                    continue
                tagmaps[keypoint[1]][keypoint[0]][joint] = people + 1
                pass
            pass
        return tagmaps
...
class DataLoader(object):
    ...
    def pair(self, image_id):
        ...
        # tagmaps
        tagmaps = self.annotationsLoader.tapmaps(output_size[1], output_size[0], keypoints_resized)
        maps = np.concatenate([heatmaps, tagmaps], axis=-1)
